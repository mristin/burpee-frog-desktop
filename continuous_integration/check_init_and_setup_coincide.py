...
"""Check that the distribution and burpeefrog/__init__.py are in sync."""
import os
import pathlib
import subprocess
import sys
...
def main() -> int:
    ...
    if setup_py_map["version"] != burpeefrog.__version__:
        print(
            f"The version in the setup.py is {setup_py_map['version']}, "
            f"while the version in burpeefrog/__init__.py is: "
            f"{burpeefrog.__version__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["author"] != burpeefrog.__author__:
        print(
            f"The author in the setup.py is {setup_py_map['author']}, "
            f"while the author in burpeefrog/__init__.py is: "
            f"{burpeefrog.__author__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["license"] != burpeefrog.__license__:
        print(
            f"The license in the setup.py is {setup_py_map['license']}, "
            f"while the license in burpeefrog/__init__.py is: "
            f"{burpeefrog.__license__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["description"] != burpeefrog.__doc__:
        print(
            f"The description in the setup.py is {setup_py_map['description']}, "
            f"while the description in burpeefrog/__init__.py is: "
            f"{burpeefrog.__doc__}",
            file=sys.stderr,
        )
        success = False
    ...
    if status_classifier is None:
        ...
        if expected_status_in_init != burpeefrog.__status__:
            print(
                f"Expected status {expected_status_in_init} "
                f"according to setup.py in burpeefrog/__init__.py, "
                f"but found: {burpeefrog.__status__}"
            )
            success = False
